...
**Large PDFs and timeouts:** `POST /upload` returns **202** as soon as each file is saved under `data/import_pending/` and a row exists in **`import_jobs`**. Rasterization and DB work run **after** that response (FastAPI background tasks, with ingest still off the event loop via a thread). The SPA **polls `GET /import/jobs/{id}`** until `completed` or `failed`, so proxies and browsers are not holding one long HTTP request for the whole job. Render’s free tier can still **spin down** an idle instance; if a job runs longer than the idle window, it may be interrupted—use **`SLIDE_RENDER_DPI`** (e.g. `100`), smaller PDFs, or a paid/always-on service for very large files.

**Logs:** After deploy you should see **`upload queued import job_id=…`** right after **`upload read … bytes`**, then **`ingest start`** in the background. Uvicorn’s access log line for **`POST /upload`** appears when the **202** is sent (not when processing finishes). Poll traffic hits **`GET /import/jobs/…`** while the job runs.
